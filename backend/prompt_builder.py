    ...
    return [
        {
            ...
            "content": (
                ...
                "你不是旁白总控，也不能替 NPC 回答。必须符合主角的人设、状态、心情、位置、能力、物品和当前目标。"
    ...
    return [
        {
            ...
            "content": (
                ...
                "NPC 必须符合自己的性格、心情、关系、能力、装备、物品、目标和题材设定。"
                ...
                "不要让所有启用子 Agent 的 NPC 都默认出场。必须先结合当前地点、角色当前位置、最近剧情、用户行动、主角输出、事件记录判断哪些 NPC 自然在场、会抵达、会通讯或需要反应。"
    ...
    return [
        {
            ...
            "content": (
                ...
                "不得推翻 hard_canon 设定，不得让角色凭空获得装备、物品、能力、情报。"
                "根据 Game.genre、world_type、tone 判断题材，不要写串。角色只能使用数据库中确认拥有的物品。"
                "如果角色没有该物品，不得描写其使用该物品。"
                ...
                f"{NARRATIVE_FORMAT_RULES}"
                "输出必须是 JSON: {\"visible_story\":\"...\"}，visible_story 字符串内也必须遵守上述排版规则。"
            ),
        },
        {
            "role": "user",
    ...
    return [
        {
            ...
            "content": (
                ...
                "如果游戏上下文包含 retrieved_memories，它们是本轮检索到的相关世界观、角色记忆和历史事件，应作为开场连续性的参考。"
    ...
    return [
        {
            ...
            "content": (
                ...
                "不要编造未发生的变化。必须记录物品获得、消耗、转移、装备、卸下、损坏、丢失、位置变化。"
                "你还要判断新出现的人物是否应该进入可管理角色表。"
                "重要 NPC 判定：有明确姓名或独立身份、会反复出场、与主角存在关系/目标/冲突、掌握关键情报、对剧情有持续影响。"
                "重要 NPC 放入 new_characters，字段包括 name, role_type, gender, age, race_or_identity, appearance, personality, speech_style, abilities, current_location, status, mood, relationship_to_player, relationship_score, current_goal, hidden_goal, memory_summary, agent_enabled, importance, management_reason, extra_attrs。"
                "agent_enabled 默认 true；如果只是可管理但不需要单独子智能体，可设 false。"
                "背景角色/群体判定：路人、一群军人、猎户们、店员、守卫、围观者、无名群众、一次性功能角色。"
                "背景角色/群体放入 ambient_characters，只记录 label, category, summary, location, role_in_scene, importance，不要放入 new_characters。"
                "新物品放入 new_items，字段包括 name, item_type, description, status, rarity, quantity_limit, is_stackable, is_equippable, is_consumable, is_key_item, is_tradeable, is_unique, usable_condition, effect_description, current_location, importance, extra_attrs。"
                "库存变化放入 inventory_updates，字段包括 action, item_name 或 item_id, quantity, owner_type, owner_name, owner_id, to_owner_type, to_owner_name, to_owner_id, item_state, equipped, storage_location, context。"
                "库存 action 使用 add/gain/obtain, remove/lose, consume, transfer, equip, unequip, set_state 之一。"
                "只输出 JSON，必须包含这些顶层字段：current_state_update, new_events, new_characters, ambient_characters, updated_characters, new_items, updated_items, inventory_updates, updated_story_world, player_choices。"
            ),
        },
        {
            "role": "user",
    ...
    return [
        {
            "role": "system",
            "content": (
                "你是一致性检查器。检查剧情和 state_patch 是否违反已有存档，尤其检查角色是否使用了自己没有的物品、"
                "物品数量是否足够、装备消耗转移是否符合库存记录、关键物品是否无故消失。只输出 JSON。"
            ),
        },
        {
            "role": "user",
    ...
    return [
        {
            ...
            "content": (
                ...
                "create/update/delete_character, create/update/delete_item, create/update/delete_inventory_record, "
                "transfer_item, use_item, equip_item, unequip_item, create/update/delete_event, create/update/delete_template。"
                ...
                "你可以结合 recent_turns、world_events、characters 和 current_story_world 判断当前剧情里哪个 NPC 自然出现、哪个 NPC 不该出现；如果用户要求安排 NPC 出场，优先通过角色位置/目标、事件或当前状态来表达，不要强迫所有 NPC 同时出现。"
