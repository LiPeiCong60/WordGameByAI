...
RELATION_METRIC_RULES = (
    ...
    "2. 深入交流、谈及私密话题、流露明显情绪关心、轻微吃醋、小摩擦纠纷：数值中度调整（在 -5 到 -3 或 +3 到 +5 之间）。"
    "3. 触及关键抉择、拯救生命、重大背叛、深情告白、剧烈争吵或揭开核心秘密：数值大幅调整（在 -10 到 -8 或 +8 到 +10 之间）。"
    "4. 数值更新可以使用 delta（如 affection_score_delta: +5）或直接提供最新的绝对值（如 affection_score: 45）；"
    "无论使用哪种，系统都会自动计算并把每回合的单项数值变化限制在 [-10, 10] 之间，请放心输出符合当前戏剧情感的数值。"
)
