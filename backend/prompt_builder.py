...
NARRATIVE_FORMAT_RULES = (
    ...
    "1. 角色行动、场景描写、环境描写、外貌/动作描写都单独成段，并用英文中括号 [] 包起来，例如：[主角推开窗，冷风灌进房间。]。每个段落必须自闭合，严禁在同一个括号中途换行（即 [] 或 () 内部禁止插入换行符）。"
    "2. 角色心理活动单独成段，并用英文小括号 () 包起来，例如：(他意识到自己不能再拖下去。)。凡是角色的心理想法、内心独白、观察思考、细节推理分析（例如：“他注意到……”、“你心想……”），都必须单独成段，并用英文小括号 () 包裹。"
    "3. 角色说话保持普通对白，格式为 角色名：台词。严禁使用任何双引号 “” 或 \"\" 包裹台词，直接输出台词正文。角色名必须纯净，禁止在角色名后或冒号前附加任何括号或动作（如禁止写成 `程予安（笑着）：`，必须拆分成动作段 `[程予安笑了笑。]` 和对白段 `程予安：`）。"
    "4. 如果一段里既有动作又有说话，必须拆成两段：先写 [动作/描写]，再写 角色名：台词。每个段落只允许包含一个角色的说话内容，多角色轮流对话必须换行并分段输出。"
...
RELATION_METRIC_RULES = (
    ...
    "2. 深入交流、谈及私密话题、流露明显情绪关心、轻微吃醋、小摩擦纠纷：数值中度调整（在 -15 到 -5 或 +5 到 +15 之间）。"
    "3. 触及关键抉择、拯救生命、重大背叛、深情告白、剧烈争吵或揭开核心秘密：数值大幅调整（在 -50 到 -20 或 +20 到 +50 之间）。"
    "4. 数值更新可以使用 delta（如 affection_score_delta: +20）或直接提供最新的绝对值（如 affection_score: 45）；"
