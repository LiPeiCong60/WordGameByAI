...
def _strip_system_fields(data: dict, keep: set[str] | None = None) -> dict:
    keep = keep or set()
    system_fields = {"id", "game_id", "owner_user_id", "created_at", "updated_at", "applied_at"}
...
def import_game(payload: dict, db: Session, owner_user_id: int | None = None) -> Game:
    try:
        ...
        for row in payload.get("characters", []):
            db.add(Character(**_strip_system_fields(row), game_id=game.id))
        ...
        for row in payload.get("management_sessions", []):
            old_id = row.get("id")
            record = ManagementSession(
                **_strip_system_fields(row),
                game_id=game.id,
