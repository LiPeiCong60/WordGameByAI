...
from models import Game, RagMemory
...
def list_rag_memories(game_id: int, db: Session = Depends(get_session)):
...
def search_rag_memories(game_id: int, payload: RagSearchRequest, db: Session = Depends(get_session)):
    try:
...
def rebuild_game_rag(game_id: int, db: Session = Depends(get_session)):
    try:
