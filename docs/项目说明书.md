...
WordGameByAI 是一个通用 AI 文字 RPG 引擎原型。项目并不是单纯的聊天页面，而是把 AI 叙事、世界观管理、角色卡、存档导入导出和多 Agent 剧情推进整合到一个结构化游戏框架中。
...
| 动态记忆 RAG | 使用 RagMemory 保存世界观、角色长期记忆和剧情摘要，生成前检索相关记忆，生成后随剧情推进更新。 |
...
| 层级 | 技术 |
| --- | --- |
| 后端 | Python、FastAPI、SQLModel、Pydantic、MySQL/SQLite、LangChain、langchain-openai |
...
flowchart LR
    ...
    Services --> DB["MySQL 或 SQLite / SQLModel"]
    Engine --> DB
...
| 数据模型 | `backend/models.py` | 定义 Game、StoryWorld、WorldLore、Character、TurnLog 等核心表。 |
...
| 路由层 | `backend/routers/` | 按 games、characters、templates、turns、management 等功能拆分 API。 |
...
| 路由 | `frontend/src/router/index.js` | 管理页面路由，包括进行、管理、模板等页面。 |
| API 封装 | `frontend/src/api/` | 封装 Axios 请求，按业务模块拆分接口。 |
| 状态管理 | `frontend/src/stores/` | 保存当前游戏、UI 状态等前端状态。 |
| 视图页面 | `frontend/src/views/` | 游戏列表、剧情进行、综合管理、模板等页面。 |
| 组件 | `frontend/src/components/` | 角色卡、剧情日志、管理 Agent 面板等复用组件。 |
...
快速模式会在剧情正文后附带用户不可见的 `STATE_HINT`，只用于即时同步角色软状态：

- 只允许更新已存在角色和当前游戏状态。
- 可更新心情、状态、关系标签、目标、位置、记忆摘要。
- 关系、好感、信任、张力等数值每轮变化会被限制在小范围内。
- 完整 PatchAgent 和 CheckerAgent 仍在后台异步执行，用于保持质量和一致性。
...
| RAG 记忆 | `GET /api/games/{game_id}/rag/memories`、`POST /api/games/{game_id}/rag/search`、`POST /api/games/{game_id}/rag/rebuild` |
...
| `DATABASE_URL` | 数据库连接地址，公网建议 MySQL；本地开发可继续使用 SQLite。 |
...
- 管理动作只允许白名单 action，禁止任意 SQL、删除数据库或修改 API Key。
...
- 已加入基础账号、验证码、登录鉴权和管理员后台；正式公网部署前仍建议接入更强的验证码、限流、审计日志和数据库备份。
- SQLite 适合单机验证，多用户并发需要迁移到 PostgreSQL 等数据库。
- Agent 调用依赖外部 LLM，质量、延迟和费用取决于模型服务。
- 当前已实现轻量动态记忆 RAG；长篇记忆压缩、任务系统、地点系统和阵营系统仍属于后续扩展方向。

## 17. 后续规划

- 将当前本地哈希向量 RAG 升级为 Chroma / PostgreSQL pgvector，并增加长篇记忆压缩。
- 增加 PostgreSQL / pgvector 支持。
- 增加存档版本化、回档和分支剧情管理。
- 增加更强的规则引擎，减少完全依赖 LLM 的一致性判断。
- 增加任务 Agent、地点 Agent、阵营 Agent。
- 增加流式剧情生成的前端体验优化。
- 增加 Docker Compose、自动迁移和更完整的生产环境安全配置。
